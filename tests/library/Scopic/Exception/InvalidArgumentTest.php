 ...
 */
class Scopic_Exception_InvalidArgumentTest extends PHPUnit_Framework_TestCase
{
    public function testMessageFormat()
    {
        try {
            throw new Scopic_Exception_InvalidArgument($this, 'Invalid argument message');
        } catch (Scopic_Exception_InvalidArgument $exception) {
            $this->assertStringMatchesFormat('[Scopic_Exception_InvalidArgumentTest] %s', $exception->getMessage());
        }
    }
}
