 ...
 */
class Scopic_ExceptionTest extends PHPUnit_Framework_TestCase
{
    public function testMessageFormat()
    {
        try {
            throw new Scopic_Exception($this, 'Base message');
        } catch (Scopic_Exception $exception) {
            $this->assertStringMatchesFormat('[%s] %s', $exception->getMessage());
        }
    }
}
